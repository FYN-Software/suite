...
import Button, { ButtonEventsMap } from '../form/button.js';
...
export default class Dialog extends Component<Dialog>
{
    ...
    protected async ready(): Promise<void>
    {
        ...
        const listener: EventListenerConfig<Button, ButtonEventsMap> = {
            click: ({ action }) => {
                switch(action)
                {
                    case 'close':
        ...
        this.shadow.on<Button, ButtonEventsMap>('slot[name="footer"] > [action]', listener);
        this.on<Button, ButtonEventsMap>('[slot="footer"][action]', listener);
    }
